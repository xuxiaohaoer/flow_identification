...
def analyze_packet(timestamp, packet, nth):
    ...
    print(nth, eth.data.data.data[0:2])
    if isinstance(eth.data, dpkt.ip.IP):
        # print("timestamp:", _timestamp, "debug")
        parse_ip_packet(eth.data, nth, timestamp)
...
def parse_tcp_packet(ip, nth, timestamp):
    ...
    if (stream[0]) in {20, 21, 22, 23, 128, 25}:
        if (stream[0]) in {20, 21, 22, 128, 25}:
...
def parse_tls_handshake(ip, data, record_length):
    ...
    if connection in encrypted_streams:
        ...
        handshake_type = ord(data[:1]) # 握手协议类型
        ...
        while total_len_consumed < record_length:
            ...
            # print("handshake.type:", handshake_type)
            ...
            if handshake.type == 2:
                ...
                feature.ip_des=client
                feature.ip_sou=server
                if need_more_parse:
...
def parse_server_hello(handshake):
    ...
    #                                              cipher_suite)))
    feature.size += len(handshake) # 特征值 TLS server hello 长度
    feature.flow_num += 1
        ...
        with open(filename, 'rb') as f:
            ...
            for timestamp, packet in capture:
                ...
                if i == 1:
                    ...
                    # print(i, 0)
                # else:
                #     print(i, timestamp-flag)
                time = timestamp -flag
                i += 1
            feature.pack_num = i
            feature.time = time
            print(feature.pack_num, feature.time, feature.ip_sou, feature.ip_des, feature.flow_num, feature.size)
            process_left_buffer()
            # print("*" * 99)
